...
(function() {
  ...
  handlebars.registerHelper('makeList', function(arr) {
    var list;
    list = _.map(arr, function(item) {
      return "'" + item + "'";
    });
    ...
    Package.prototype.resolve = function(file) {
      ...
      return script.deps.forEach((function(_this) {
        return function(f) {
          ...
          if (__indexOf.call(_.pluck(_this.files, 'name'), f) >= 0) {
            return;
          }
          return _this.resolve(join(_this.basePath, f));
        };
      })(this));
    };
